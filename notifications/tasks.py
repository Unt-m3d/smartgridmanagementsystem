    ...
    try:
        ...
        if sendgrid_api_key and sendgrid:
            ...
            try:
                ...
                logger.warning(f"SendGrid error: {str(sg_error)}. Falling back to Django email.")
...
def send_sms_alert(self, phone, message):
    ...
    try:
        ...
        if not all([twilio_account_sid, twilio_auth_token, twilio_phone_number]):
            logger.warning("Twilio credentials not configured. SMS alert skipped.")
            return False
...
def check_energy_alerts():
    ...
    try:
        ...
        try:
            ...
            logger.warning("No energy data available for alert check")
            return False
        ...
        if not alert_rules.exists():
            logger.info("No active alert rules configured")
            return True
        ...
        for rule in alert_rules:
            try:
                ...
                logger.error(f"Error processing alert rule {rule.id}: {str(rule_error)}")
                continue
