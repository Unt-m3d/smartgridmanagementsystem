    ...
    try:
        ...
        if sendgrid_api_key and sendgrid:
            try:
                ...
                logger.warning(f"⚠️ SendGrid failed: {str(sg_error)}. Trying Gmail...")
...
def send_sms_alert(self, phone, message):
    ...
    try:
        ...
        if not all([twilio_account_sid, twilio_auth_token, twilio_phone_number]):
            logger.warning("⚠️ Twilio not configured. SMS skipped. Add credentials to .env")
            return False
...
def check_energy_alerts():
    ...
    try:
        ...
        try:
            ...
            logger.info("⚠️ No energy data available for alert check")
            return False
        ...
        if latest_data.voltage > high_voltage_threshold:
            alerts_triggered.append({
                'type': 'HIGH_VOLTAGE',
                'message': f"⚠️ HIGH VOLTAGE ALERT: {latest_data.voltage:.2f}V exceeds {high_voltage_threshold}V",
                'severity': 'HIGH'
            })
        ...
        if latest_data.voltage < low_voltage_threshold:
            alerts_triggered.append({
                'type': 'LOW_VOLTAGE',
                'message': f"⚠️ LOW VOLTAGE ALERT: {latest_data.voltage:.2f}V below {low_voltage_threshold}V",
                'severity': 'HIGH'
            })
        ...
        if latest_data.power > high_power_threshold:
            alerts_triggered.append({
                'type': 'HIGH_POWER',
                'message': f"⚠️ HIGH POWER ALERT: {latest_data.power:.2f}W exceeds {high_power_threshold}W",
                'severity': 'MEDIUM'
            })
        ...
        if latest_data.current > high_current_threshold:
            alerts_triggered.append({
                'type': 'HIGH_CURRENT',
                'message': f"⚠️ HIGH CURRENT ALERT: {latest_data.current:.2f}A exceeds {high_current_threshold}A",
                'severity': 'MEDIUM'
            })
